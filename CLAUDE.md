...
2. Traditional production can be beautiful, but slow, expensive and tedious. (sub bridges into the 5 problem icon-cards; was "Great products deserve better visual velocity.") The left `.card` shows a real "traditional shoot" photo (a cropped `object-fit:cover` band in a `height:clamp(130px,19vh,190px)` box, replacing the old inline SVG illustration) over a one-line caption; the right column is a 2x2 grid of 4 problem `.icard`s. Photo is `assets/shoot-jewellery.jpg` (jewellery), swapped by the build scripts to root-absolute `/assets/shoot-furniture.jpg` and `/assets/shoot-fashion.jpg` for the subfolder decks. Keep the sub and caption to one line each so the card fits inside `.grow` at 16:9 without overlapping the headline.
