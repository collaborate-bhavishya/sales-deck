...
5. We specialise in Jewellery. — a compact centered `.jstrip` row (max-width 680px) of 4 category images (spec-ring, spec-bangle, spec-necklace, spec-rings) sits above the 4 `.card` text boxes; `.jtile img` are wired into the `#lightbox` (cycle within the strip).
