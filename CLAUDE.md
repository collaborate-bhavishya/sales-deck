...
5. We specialise in Jewellery. — a `.jstrip` row of 4 category images (spec-ring, spec-bangle, spec-necklace, spec-rings) sits above the 4 `.card` text boxes, aligned to the same 4-column grid; `.jtile img` are wired into the `#lightbox` (cycle within the strip).
